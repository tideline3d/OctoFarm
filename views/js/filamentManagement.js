...
async function init () {
    ...
    filamentManager = checkFilamentManager();
    if (filamentManager) {
        ...
        resyncBtn.addEventListener('click', async e => {
          ...
          '<i class="fas fa-sync"></i> Re-Sync Filament Manager';
            if (post.status === 200) {
                ...
                console.log(meta);
                if (meta.success) {
                    UI.createAlert('success', `Successfully synced filament manager! <br> Profiles - Updated: ${meta.updatedProfiles} / New: ${meta.newProfiles} <br> Spools - Updated: ${meta.updatedSpools} / New: ${meta.newSpools}`, 4000, 'Clicked');
                } else {
                    UI.createAlert('error', `Successfully synced filament manager! <br> Profiles Status: ${meta.profiles} <br> Spools Status: ${meta.spools}`, 4000, 'Clicked');
                }
                resyncBtn.disabled = false;
            } else {
                UI.createAlert('error', 'Could not contact system to sync, is it online?');
                resyncBtn.disabled = false;
            }
        });
    }
