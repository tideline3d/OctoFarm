...
class ServerSettings {
    static async init() {
        Client.get("settings/system/get")
            .then((res) => {
                return res.json();
            })
            .then((res) => {
        ...
        let logList = await Client.get("settings/system/get/logs");
    ...
    static update() {
      ...
      oldServerSettings.timeout.apiRetry !== timeout.apiRetry
        ...
        Client.post("settings/system/update", {
            onlinePolling,
            server,
            timeout,
            filament,
            ...
            .then((res) => {
              ...
              "Your settings changes require a restart, would you like to do this now?",
                        ...
                        callback(result) {
                            if (result) {
                                OctoFarmclient.get("settings/system/restart");
                                UI.createAlert(
                                    "warning",
                                    "Performing a system restart, please wait for it to come back online.",
                                    6000,
                                    "Clicked"
                                );
                            }
