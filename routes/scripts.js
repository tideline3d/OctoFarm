...
const script = require("../systemRunners/system/scriptCheck.js");
...
router.post("/test", ensureAuthenticated, async (req, res) => {
    ...
    const testFire = await Script.test(opts.scriptLocation, opts.message);
    //Return printers added...
...
router.post("/save", ensureAuthenticated, async (req, res) => {
    ...
    const opts = req.body;
    //Send Dashboard to Runner..
    const save = await Script.save(opts.printer, opts.trigger, opts.message, opts.scriptLocation);
    //Return printers added...
...
router.post("/edit", ensureAuthenticated, async (req, res) => {
    ...
    const opts = req.body;
    //Send Dashboard to Runner..
    const save = await Script.edit(opts);
    //Return printers added...
    res.send({message: save, status:200});
});
